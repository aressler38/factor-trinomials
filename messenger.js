...
Messenger = (function(){
    ...
    return ({
        on: function(trigger, handler, context) {
            ...
            events[trigger].context = context;
        ...
        send: function(event, dataThru) {
            ...
            if (events[event] && events[event].handler) {
                if (events[event].context) {
                    return events[event].handler.apply(events[event].context, events[event].data);
                }
                else {
                    return events[event].handler.apply(this, events[event].data);
                }
            }
        }
    });
